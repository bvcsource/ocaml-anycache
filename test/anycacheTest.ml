...
module Make(Monad: MonadTest) = struct
  ...
  let test_cache_bind_seq () =
    ...
    check float "cache value1" val_d v1;
    let v2 = Monad.run (fun () -> lookup cache key1 (compute_data_direct cnt)) in
    check float "cache value2" val_d v2;
  ...
  let test_cache_size n () =
    ...
    Monad.run (fun () ->
      ...
      check (option float) "still in cache(2)" (Some val_p) second';
...
module MakePending(Monad:sig
  ...
  module PL = Anycache_pendinglimit.Make(String)(Monad)
