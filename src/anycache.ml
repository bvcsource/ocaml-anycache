...
module Make(K: Map.OrderedType)(M : Monad) = struct
  module Anycache_LRUMap = Anycache_LRU.Make(K)
  type 'a t = 'a M.t Anycache_LRUMap.cache

  type key = K.t
  type 'a deferred = 'a M.t
  type 'a validator = (K.t * 'a option) -> 'a M.t

  let create = Anycache_LRUMap.create

  open! M
  let find cache key ~f =
    match Anycache_LRUMap.find cache key with
    | None -> f (key,None)
    | Some old ->
        old >>? function
  ...
  let set cache key value =
    Anycache_LRUMap.replace cache key (M.return value)
  ...
  let with_validator cache (revalidate: 'a validator) key : 'a M.t =
    ...
    Anycache_LRUMap.replace cache key pending;
    pending >>? function
    | Ok r -> M.return r
    | Error e -> M.fail e
